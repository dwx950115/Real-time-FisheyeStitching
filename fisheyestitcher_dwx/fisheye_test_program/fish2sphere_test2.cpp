...
main( int argc, char** argv )
{
    ...
    fish_common_init(map_x, map_y, cir_mask, scale_map, mls_map_x, mls_map_y, 
                     binary_mask, blend_post, Hs, Ws, Hd, Wd, 210);

    Mat res_mls_map_x(1, 1645, mls_map_x.type());
    resize(mls_map_x, res_mls_map_x, res_mls_map_x.size(), INTER_LINEAR);
    Mat res_mls_map_y(1, 1645, mls_map_y.type());
    resize(mls_map_y, res_mls_map_y, res_mls_map_y.size(), INTER_LINEAR);
    // 
    in_img.release(); 
    ...
    while( 1 )
    {
        ...
        fish_stitch_one(pano, in_img_L, in_img_R, cir_mask, scale_map, 
                        map_x, map_y, res_mls_map_x, res_mls_map_y, Hs, Ws, Hd, Wd, 
