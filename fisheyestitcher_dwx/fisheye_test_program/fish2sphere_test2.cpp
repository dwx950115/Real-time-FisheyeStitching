                    ...
                    float cam1_u0, float cam1_fx, float cam2_u0, float cam2_fx)
{
    ...
    Mat left_img_crop_left = left_unwarped(Rect(imW*8/36,0,imW*2/36,imH));
    Mat left_img_crop_right = left_unwarped(Rect(imW*26/36,0,imW*2/36,imH));
    Mat right_img_crop_left = right_unwarped(Rect(imW*8/36,0,imW*2/36,imH));
    Mat right_img_crop_right = right_unwarped(Rect(imW*26/36,0,imW*2/36,imH));
    int sideW = imW*2/36/2;
#if TEST
    ...
    Mat part1 = left_unwarped(Rect(imW*18/36,0,imW*8/36,imH));
    Mat part2 = left_img_crop_right;
    Mat part3 = right_unwarped(Rect(imW*10/36,0,imW*16/36,imH));
    Mat part4 = left_img_crop_left;
    Mat part5 = left_unwarped(Rect(imW*10/36,0,imW*8/36,imH));
    ...
    ***************************************************************************************************/
    ...
    while (true)
	{
        ...
        fish_blend_directly(left_unwarped,right_unwarped,pano,
                            cam1_u0, cam1_fx, cam2_u0, cam2_fx);
		...
		// uint16_t crop = uint16_t(0.5 * Ws * (200.0 - 180.0) / 200.0); // half overlap region
	    // // --------------------------------------------------------------------------
